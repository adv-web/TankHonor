...
public class TankShoot : PunBehaviour
{
    ...
    public Transform mainGunTransform;          //枪械对象的Transform属性
    public Transform Barrel_MainPart;           //主炮炮管对象的Transform属性
    ...
    /*public Vector3 shootingPosition         //枪械射击位置
         = new Vector3(0.00529028f, -2.48354f, 0.987773f);*/
    ...
    void Start()
    {
        tankHealth = GetComponent<TankHealth>();	//获取玩家TankHealth组件
        myCamera = Camera.main;
        timer = 0.0f;
        gunInitLocalPos = Barrel_MainPart.localPosition;//初始化玩家与上次射击的间隔
    ...
    void Update()
    {
        ...
        {   //玩家按下射击键，与上次射击的间隔超过射击间隔
            ...
            Shoot(mainGunTransform.position);
            //Barrel_MainPart.localPosition = gunInitLocalPos + (Barrel_MainPart.position - gunBarrelEnd.position) * 0.3f * recoilForce;
            Barrel_MainPart.localPosition = gunInitLocalPos + (Barrel_MainPart.forward) * 0.5f * recoilForce;
    ...
    void posRectify()
    {
        Barrel_MainPart.localPosition = Vector3.Lerp(Barrel_MainPart.localPosition, gunInitLocalPos, Time.deltaTime * 2.0f);
    }
    //每一帧射出射线检测
    void enemyInfoCheck()
    {
       // Vector3 rotatedPos = transform.rotation * shootingPosition; //随着角色的旋转，射击的初始位置也需要跟着旋转
        ray.origin = Barrel_MainPart.position;	//设置射击射线的起始端点
        ray.direction = gunBarrelEnd.position - Barrel_MainPart.position;				//设置射击射线的方向
        ...
        if (Physics.Raycast(ray, out hitInfo, shootingRange))
        {
            ...
            if (go.tag == "Player")//如果击中玩家
            {
                ...
                if (tankHealth.team != GetComponent<TankHealth>().team)
                {
    ...
    void Shoot(PhotonPlayer attacker, Vector3 shootPosition)
    {
        ...
        ray.direction = mainGunTransform.forward;				//设置射击射线的方向
        ...
        {   //如果射线击中游戏对象
            ...
            else if (go.tag == "Zombie" || go.tag == "Guard" || go.tag == "Skeleton")
            {                       //如果击中僵尸
                ZombieHealth zh = go.GetComponent<ZombieHealth>();
                if (zh != null)
                {
    ...
    void Shoot(Vector3 shootPosition)
    {
        ...
        //photonView.RPC("ShootEffect", PhotonTargets.AllViaServer, bulletEffectPosition, hitTag);//使用RPC，调用所有玩家对象的ShootEffect函数，显示射击效果
        ShootEffect(bulletEffectPosition);
    }
    //RPC函数，显示射击效果
    /*[PunRPC]
